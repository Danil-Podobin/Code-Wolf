...
window.wallpaperPropertyListener = {
  applyUserProperties: properties => {
	...
	if (properties.customimageleft) {
                if (properties.customimageleft.value) {
                    // Create a valid location path
                    var customImageFile = "file:///" + properties.customimageleft.value;
                } else {
                    // You may want to reset the background when the user clears the Image
                    customImageFile = "http://res.cloudinary.com/dub9ykyuq/image/upload/v1517697588/Untitled_wcfedl.png";
                }
            imageElement = document.getElementById('LeftImage');
            imageElement.src = customImageFile;
        };
	if (properties.customimageright) {
                if (properties.customimageright.value) {
                    // Create a valid location path
                    var customImageFile = "file:///" + properties.customimageright.value;
                } else {
                    // You may want to reset the background when the user clears the Image
                    customImageFile = "http://res.cloudinary.com/dub9ykyuq/image/upload/v1517696936/wolfsvg_hxf2jz.png";
                }
            imageElement = document.getElementById('RightImage');
            imageElement.src = customImageFile;
        };
	if (properties.click) {
if (properties.click.value == true) {
document.addEventListener("click", changeWidth);
}
else {
document.removeEventListener("click", changeWidth);
};
}
